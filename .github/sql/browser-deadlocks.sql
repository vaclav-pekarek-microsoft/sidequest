...
DECLARE @buffer xml = (
    ...
    WHERE session.name = N'SidequestBrowserDiagnostics' AND target.target_name = N'ring_buffer'
);
IF @buffer IS NULL
    THROW 50000, 'The isolated browser ring buffer is unavailable for diagnostics.', 1;
