...
internal sealed class InvitationReadGate : DbCommandInterceptor
{
    ...
    private static bool IsInvitationRead(DbCommand command) =>
        command.CommandText.StartsWith("SELECT", StringComparison.OrdinalIgnoreCase) &&
        command.CommandText.Contains("FROM [QuestInvitations]", StringComparison.Ordinal) &&
        command.CommandText.Contains("[InvitedById]", StringComparison.Ordinal);
}
